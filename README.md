...
不再等待 ROS 节点发现、V4L2 打开和摄像头首帧。摄像头进程退出或画面中断时会自动重启，
主程序退出时才释放物理摄像头。
...
Web 配置中的 `launch.tracking` 决定是否加载视觉跟踪能力。物理摄像头随主程序启动并保持热备；开启跟踪能力时，RDK 编解码、补边和 BPU 检测进程也会预先启动，但在没有摄像头租约时收不到帧，不持续做推理。语音模型识别到“看着我”“跟着我”等意图并调用视觉工具后，只开启帧转发和结果使用；退出跟踪后关闭帧转发，但不会反复关闭、重开摄像头或重新加载检测模型。旧配置项 `vision.enabled_on_start` 仅为兼容保留，不再控制运行状态。
