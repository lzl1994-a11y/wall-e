...
`wali_bt_ros2_bridge` 同时使用官方 `btcpp_ros2_interfaces/action/ExecuteTree` 提供 Action Server
...
运行 `tools/build_behaviortree_ros2.sh` 会单线程构建官方接口、包装库和桥接节点。桥接阶段
直接使用 `rclcpp_action` 管理生命周期；官方 `TreeExecutionServer` 在 Humble 分支执行过
目标后退出会遗留可 join 线程，因此暂不用于常驻进程，后续升级到包含修复的版本再切换。
...
没有租约时通过 `/set_capture` 执行 V4L2 `STREAMOFF`，保留进程、设备句柄和配置但不再取帧。
视觉问答、拍照或跟踪请求到来时执行 `STREAMON` 并恢复 `/camera_frame` 转发，无需重新启动
ROS 节点或重新打开 V4L2 设备。摄像头进程退出或取流期间画面中断时会自动重启，主程序退出时
才释放物理摄像头。
...
把 MP3、WAV、FLAC、AAC、M4A 或 OGG 文件放入 `/root/wall-e/music`（也可用
...
下位机协议和固件无需修改。音乐、TTS 与多模态唤醒提示音由播放节点在同一个输出流中混音；
...
Web 配置中的 `launch.tracking` 决定是否加载视觉跟踪能力。物理摄像头随主程序启动并保持热备；开启跟踪能力时，RDK 编解码、补边和 BPU 检测进程也会预先启动，但在没有摄像头租约时 UVC 处于 `STREAMOFF`，不取帧也不持续做推理。语音模型识别到“看着我”“跟着我”等意图并调用视觉工具后，通过 `STREAMON` 快速恢复画面和结果使用；退出跟踪后重新停流，但不会反复关闭设备、重启摄像头进程或重新加载检测模型。旧配置项 `vision.enabled_on_start` 仅为兼容保留，不再控制运行状态。
...
视觉图片分析不提供任何动作工具，使用专用视觉提示词和可见答案过滤后读取标准 `content`，以兼容不能稳定返回 `direct_answer` 的模型。摄像头图片只附加到当前视觉请求，不写入对话历史；后续上下文仅保留用户问题和文字答案，并会在服务边界剥离遗留的图片或音频块。原始音频多模态直聊仍保留 `direct_answer(response)` 结构化出口。
