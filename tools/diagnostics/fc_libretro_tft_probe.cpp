...
class AudioOut {
 public:
  ...
  void push(const int16_t* p,size_t frames){ std::lock_guard<std::mutex> l(mu_); for(size_t i=0;i<frames*2;i++){ int v=int(p[i])*4/5; q_.push_back(int16_t(v)); } while(q_.size()>19200) q_.pop_front(); cv_.notify_one(); }
 private:
