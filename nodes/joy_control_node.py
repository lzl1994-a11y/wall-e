...
from services.motor_control import compute_joystick_motor_decision
...
class JoyControlNode(Node):
    ...
    def _tick_loop(self):
        ...
        motor_decision = compute_joystick_motor_decision(
            ly, lx, self._was_moving
        )
        if motor_decision.should_stop:
            self._stop_motors()
        self._was_moving = motor_decision.is_moving
        if motor_decision.command is not None:
            msg_m = String()
            msg_m.data = json.dumps(motor_decision.command)
