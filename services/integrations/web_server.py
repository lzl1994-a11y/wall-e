...
# English: Direct script execution starts with only this directory on sys.path.
# 中文：直接执行本脚本时 sys.path 只有当前目录，因此显式加入项目根目录。
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from services.hardware.usb_devices import USB_ROLES, list_usb_devices
