...
from services.audio.alsa_capture import ArecordInputStream, native_capture_available
from services.hardware.usb_devices import resolve_alsa_capture_device, resolve_audio_device
...
class AudioPipeline:
    ...
    def _init_silero_vad(self):
        ...
        if not os.path.isabs(model_path):
            # English: audio_pipeline.py is now under services/audio; walk three
            # levels up so configured model paths remain repository-relative.
            # 中文：文件分类到 services/audio 后需向上三级，确保配置中的模型路径
            # 仍相对于仓库根目录解析，而不是相对于 services 目录。
