...
def _load_sequence_names():
    path = Path(__file__).resolve().parents[2] / "core" / "sequences.yaml"
    try:
...
def _valid_arguments(name, arguments):
    ...
    if name == "express_emotion":
        ...
        try:
            from services.vision.visual_search import normalize_visual_search_arguments
        ...
        try:
            from services.orchestration.conditional_task import normalize_conditional_task_plan
