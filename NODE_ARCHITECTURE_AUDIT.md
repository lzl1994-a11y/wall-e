...
| `camera_capture_node.py` / `camera_capture_node` | Default | P `/camera_frame`, `/camera_capture_status`; S `/camera_capture_cmd`, `/image`; C `/set_capture` | Starts, health-checks, restarts, and stops the single `hobot_usb_cam`; leases switch V4L2 `STREAMON/OFF` while the process and device configuration stay warm. **Sole physical camera/V4L2 owner.** | `camera_capture_protocol`, `usb_devices`. Process, stream, and lease boundaries correctly remain in Node. |
